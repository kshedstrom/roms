...
!
     ...
     &                        cg_QG)
        ...
        CALL netcdf_get_fvar (ng, iTLM, LCZname(ng), 'cg_zv',           &
     &                        cg_zv)
        IF (exit_flag.ne. NoError) RETURN

        CALL netcdf_get_fvar (ng, iTLM, LCZname(ng), 'cg_Ritz',         &
     &                        cg_Ritz)
        IF (exit_flag.ne. NoError) RETURN
...
!
        lstr=LEN_TRIM(TLMbase(ng))
        WRITE (TLMname(ng),10) TLMbase(ng)(1:lstr-3), outer+2
!
!  Clear tangent linear forcing arrays before entering inner-loop.
!  This is very important since these arrays are non-zero after
!  running the representer model and must be zero when running the
...
!
        lstr=LEN_TRIM(TLMbase(ng))
        WRITE (TLMname(ng),10) TLMbase(ng)(1:lstr-3), outer+3
!
        FrequentImpulse=.FALSE.
!
!  Initialize tangent linear model from ITLname, record 1.
...
!
        lstr=LEN_TRIM(TLMbase(ng))
        WRITE (TLMname(ng),10) TLMbase(ng)(1:lstr-3), outer+4
!
        FrequentImpulse=.FALSE.
!
!  Initialize tangent linear model from ITLname, record 1.
...
!
        lstr=LEN_TRIM(TLMbase(ng))
        WRITE (TLMname(ng),10) TLMbase(ng)(1:lstr-3), outer+5
!
!  Set basic state trajectory.
!
        lstr=LEN_TRIM(FWDbase(ng))
...
!
          lstr=LEN_TRIM(TLMbase(ng))
          WRITE (TLMname(ng),10) TLMbase(ng)(1:lstr-3), outer+6
!
!  Assigne logical flag to turn on weak constraint forcing for model error.
!
          FrequentImpulse=.TRUE.
!
!  Initialize tangent linear model from ITLname, record 1.
