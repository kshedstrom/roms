...
!
!! svn $Id: ana_hmixcoef.h 205 2007-04-21 22:28:17Z arango $
!!================================================= Hernan G. Arango ===
...
#  if defined UV_VIS2
      DO i=IstrR,IendR
        DO j=JstrR,MIN(Iwrk,JendR)
          cff = 1000.*0.5_r8*(1.0_r8+COS(pi*REAL(j,r8)/REAL(Iwrk,r8)))
          visc2_r(i,j) = max(cff, visc2_r(i,j))
          visc2_p(i,j) = max(cff, visc2_p(i,j))
        END DO
      END DO
      DO i=IstrR,MIN(Iwrk,IendR)
        DO j=MAX(JstrR,i),JendR
          cff = 1000.*0.5_r8*(1.0_r8+COS(pi*REAL(i,r8)/REAL(Iwrk,r8)))
          visc2_r(i,j) = max(cff, visc2_r(i,j))
          visc2_p(i,j) = max(cff, visc2_p(i,j))
        END DO
      END DO
