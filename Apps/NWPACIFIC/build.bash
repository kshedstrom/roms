...
export   ROMS_APPLICATION=NWPACIFIC

# Set a local environmental variable to define the path to the directories
# where all this project's files are kept.

export        MY_ROOT_DIR=${HOME}/MODELS/ROMS-FISH.git
    ...
    gfortran )
      ...
      if [ -n "${USE_NETCDF4:+1}" ]; then
        if [ -n "${USE_PARALLEL_IO:+1}" ] && [ -n "${USE_MPI:+1}" ]; then
          ...
          # triton
          export       NC_CONFIG=/usr/local/esm-gfortran/netcdf/netcdf/bin/nc-config
          export   NETCDF_INCDIR=/usr/local/esm-gfortran/netcdf/netcdf/include
        fi
      else
        # triton
        export     NETCDF_INCDIR=/usr/local/esm-gfortran/netcdf/netcdf/include
        export     NETCDF_LIBDIR=/usr/local/esm-gfortran/netcdf/netcdf/lib
      fi
      ;;
...
# etc).
  ...
  export     MY_HEADER_DIR=${MY_PROJECT_DIR}/mysrc
  export MY_ANALYTICAL_DIR=${MY_PROJECT_DIR}/mysrc


# Put the binary to execute in the following directory.

 export            BINDIR=/t1/scratch/raphael/tmpdir_NWPACIFIC
